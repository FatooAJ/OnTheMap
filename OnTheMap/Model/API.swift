...
class API {
    ...
    func gettingUserData(userKey:String!){
        ...
        let task = session.dataTask(with: request) { data, response, error in
            ...
            let newData = data?.subdata(in: range) /* subset response data! */
            let myJSON =  try! JSONSerialization.jsonObject(with: newData!, options: .allowFragments) as! NSDictionary
            User.UserInfo.firstname = myJSON["first_name"] as! String
            User.UserInfo.lastname = myJSON["last_name"] as! String
    ...
    func postingStudentLocation(mapString: String, MediaURL: String, latitude: Double, longitude: Double, completion: @escaping (Bool,Error?)->()){
        print("{\"uniqueKey\": \"\(User.UserInfo.key!)\", \"firstName\": \"\(User.UserInfo.firstname!)\", \"lastName\": \"\(User.UserInfo.lastname!)\",\"mapString\": \"\(mapString)\", \"mediaURL\": \"\(MediaURL)\",\"latitude\": \(latitude), \"longitude\": \(longitude)}")
        var request = URLRequest(url: URL(string: "https://parse.udacity.com/parse/classes/StudentLocation")!)
        request.httpMethod = "POST"
        request.addValue("QrX47CA9cyuGewLdsL7o5Eb8iug6Em8ye0dnAbIr", forHTTPHeaderField: "X-Parse-Application-Id")
        request.addValue("QuWThTdiRmTux3YaDseUSEpUKo7aBYM737yKd4gY", forHTTPHeaderField: "X-Parse-REST-API-Key")
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        request.httpBody = "{\"uniqueKey\": \"\(User.UserInfo.key!)\", \"firstName\": \"\(User.UserInfo.firstname!)\", \"lastName\": \"\(User.UserInfo.lastname!)\",\"mapString\": \"\(mapString)\", \"mediaURL\": \"\(MediaURL)\",\"latitude\": \(latitude), \"longitude\": \(longitude)}".data(using: .utf8)
        let session = URLSession.shared
        let task = session.dataTask(with: request) { data, response, error in
            if error != nil { // Handle error…
